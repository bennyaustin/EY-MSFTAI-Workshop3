...
# Importing the lightgbm model package from Microsoft
import lightgbm
...
def split_data(data_df):
    ...
    train_data, valid_data, labels_train, labels_valid = train_test_split(features, labels, test_size=0.2, random_state=0)
    train_data = lightgbm.Dataset(train_data, label=labels_train)
    valid_data = lightgbm.Dataset(valid_data, label=labels_valid, free_raw_data=False)   
...
def get_model_metrics(model, data):
    ...
    ## TODO
    predictions = model.predict(data[1].data)  #data[1] is validation data !!!
